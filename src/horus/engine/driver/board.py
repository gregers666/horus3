...
class Board(object):
    ...
    def connect(self):
        ...
        self._is_connected = False
        try:
            self._serial_port = serial.Serial(self.serial_name, self.baud_rate, timeout=5)
            if self._serial_port.isOpen():
                self._reset()  # Force Reset and flush
                version = self._serial_port.readline().decode()
                #print(version)
                if "Horus 0.1 ['$' for help]" in version:
                    ...
                    logger.info("connect set to True")
                    # Set current position as origin
                    logger.info("Setting current position as origin")                    
                    self.motor_reset_origin()
                    logger.info("Origin set - Horus 0.2 connection done")
                else:
                    raise WrongFirmware()
            else:
    ...
    def read(self, read_lines=False):
        if read_lines:
            all_lines=''
            for lines in self._serial_port.readlines():
                all_lines += (lines.decode() + ' ')
            #print('all_lines=%s' % all_lines)
            return all_lines
#            return ''.join(self._serial_port.readlines())
        else:
            return ''.join(self._serial_port.readline().decode())
