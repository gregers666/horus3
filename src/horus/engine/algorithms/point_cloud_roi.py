...
class PointCloudROI(object):
    ...
    def draw_cross(self, image):
        if self._center_v != 0 and self._center_u != 0 and self._show_center:
            ...
            print("self._center_v=%s, type %s" % (self._center_v, type(self._center_v)))
            print("self._center_u=%s, type %s" % (self._center_u, type(self._center_u)))
    ...
    def _compute_roi(self):
        if self.calibration_data.check_calibration() is False:
            ...
            print("u_min=%s, type %s" % (_umin, type(_umin)))
            print("u_max=%s, type %s" % (_umax, type(_umax)))
            print("v_min=%s, type %s" % (_vmin, type(_vmin)))
            print("v_max=%s, type %s" % (_vmax, type(_vmax)))
            # Visualization
            v_ = np.array(v.T)
            #print("len(v_) = %s" % len(v_)) 
            # Lower cylinder base
            a = v_[:int((len(v_) / 2))]
            # Upper cylinder base
            b = v_[int((len(v_) / 2)):]
